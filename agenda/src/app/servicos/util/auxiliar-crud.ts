...
export class AuxiliarCrud {
    ...
    async pesquisar(id?: number) {
        if (id) {
            ...
            return await this.http.get<any[]>(this.constantes.backUrlBase + this.entidade).toPromise();
        }
    }

    async adicionar(campos: any) {
        return await this.http.post<any[]>(this.constantes.backUrlBase + this.entidade, campos).toPromise();
    }

    async atualizar(campos: any) {
        return await this.http.patch<any[]>(this.constantes.backUrlBase + this.entidade + campos.id + '/', campos).toPromise();
    }
